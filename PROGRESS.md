...
## Aktueller Stand — 2026-09-08 (mmwave_light v1.6.1: continue_on_error, Release 2.0.1)

- Anlass: Prod-Diagnose 08.09. - das SwitchBot Relay 2PM (Matter) im Flur unten antwortete
  zweimal am Tag 38 s lang nicht ("Peer is no longer responding to active session"), beide
  Blueprint-Instanzen brachen an der Schaltaktion mit Fehler ab.
- Fix: `continue_on_error: true` an allen fuenf `homeassistant.turn_on/turn_off`-Aktionen
  (bypass_off-Neubewertung ein/aus, Einschaltzweig, Timer-Ausschaltzweig, instant_off).
  Keine Logikaenderung; die Automation endet dann sauber statt mit Fehlerstatus.
- Auf Prod bereits eingespielt (Backup back/mmwave_light.yaml.bak-20260908-v160, automation.reload).
- Repo-Datei nutzt einen gefalteten description-Block (`>`), Prod-Kopie eine gequotete
  Zeichenkette - beim Portieren von Textaenderungen Zeilenumbrueche entsprechend setzen.
- VERSION 2.0.0 -> 2.0.1.
